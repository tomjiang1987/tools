...
public class App {
    ...
    public static void main(String[] args) throws IOException {
        String mapFile = "/Users/tao/projects/FlameGraph/perf.map";

        List<Pair<Pair<Long, Long>, String>> map = new ArrayList<>();
        try (Stream<String> stream = Files.lines(Paths.get(mapFile))) {
            stream.forEach((String line) -> {
                String[] params = line.split(" ");
                if (params.length == 3) {
                    long start = Long.parseLong(params[0], 16);
                    long size = Long.parseLong(params[1], 16);
                    Pair<Long, Long> pair = new Pair<>(start, start + size);
                    map.add(new Pair<Pair<Long, Long>, String>(pair, params[2]));
                }
            });
        } catch (IOException e) {
            e.printStackTrace();
        ...
        String stackFile = "/Users/tao/projects/FlameGraph/out.stack";
        String genFile = "/Users/tao/projects/FlameGraph/out.stack.gen";

        try (BufferedWriter writer = Files.newBufferedWriter(Paths.get(genFile))) {

            try (Stream<String> stream = Files.lines(Paths.get(stackFile))) {
                stream.forEach((String line) -> {

                    try {
                        if (line.indexOf(" ()") > 0) {
                            String[] params = line.trim().split("  ");
                            Long index = Long.parseLong(params[0], 16);
                            String method = "unknown";
                            for (Pair<Pair<Long, Long>, String> itr : map) {
                                Pair<Long, Long> interval = itr.getKey();
                                if (index >= interval.getKey() && index <= interval.getValue()) {
                                    method = itr.getValue();
                                }
                            }

                            writer.write(line.replace(" ()", method + " ()"));
                        } else {
                            writer.write(line);
                        }
